...
class model:
    ...
    def generate_ports(self, ports):
        # Generate inputs and outputs for template replacement
        inputs = ",\n ".join([f" {port.type} [{port.size-1}:0] {port.name}" for port in ports if port.type != port_type.OUT])
        outputs = ",\n ".join([f" {port.type} [{port.size-1}:0] {port.name}" for port in ports if port.type == port_type.OUT])
        print(f"Inputs: {inputs}")
        print(f"Outputs: {outputs}")
        
        # Generate instance inputs and outputs for template replacement
        instance_inputs = ",\n ".join([f" .{port.name}({port.name})" for port in ports if port.type != port_type.OUT])
        instance_outputs = ",\n ".join([f" .{port.name}({port.name})" for port in ports if port.type == port_type.OUT])
